...
def compute_z_level(lev, sp, t_level, q_level, z_h):
    ...
    t = (t_level * (1.0 + 0.609133 * q_level)).copy()
...
def calculate_interpolated_pressure_field(data_var_on_ml, data_p_on_ml, plevs):
    ...
    profile_count = 0
    interpolated_values = []
    for _point in range(no_grid_points):
        offset = count * nlevs
        ...
        count = count + 1
    interpolated_field = np.asarray(interpolated_values).reshape(
        nlats, nlons, len(plevs)
    )
    ...
    for var_name in list(dc.keys()):
        if var_name in ["z", "t", "q", "lnsp"]:
            ...
            if all(value in lnsp_dims for value in dc[var_name].dims):
                ...
                print(
                    "Not all required lnsp dimensions found -exiting ",
                    dc[var_name].dims,
                )
                checks = False
    return checks
...
def ECMWFml2pl(input_fname, output_fname, plevels):
    ...
    data_cube["gph"] = xr.DataArray(
        data=np.zeros(data_cube["pml"].shape),
        dims=data_cube["pml"].dims,
    ...
    for time_step in range(len(data_cube["t"].time)):
        ...
        for lev in range(len(data_cube["t"].level), 0, -1):
            ...
            z_h, data_cube["gph"][time_step, lev - 1, :, :] = compute_z_level(
                lev, sp, t_level, q_level, z_h
            )
    ...
    all_interpolated_var_fields["pressure"].attrs = {
        "units": "Pa",
        "long_name": "pressure",
        "standard_name": "air_pressure",
        "positive": "down",
    }
    all_interpolated_var_fields.to_netcdf(
        output_fname,
        format="NETCDF4",
        engine="netcdf4",
